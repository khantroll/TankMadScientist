...
- **Configure AI:** draft PR #11 https://github.com/khantroll/TankMadScientist/pull/11 on `cursor/configure-ai-settings-62a8` (`f295d0a`). Operators edit non-secret provider settings, see key status, and probe a provider without hand-editing `providers.yaml`. Secrets stay in the environment or gitignored `providers.local.yaml`. Do not merge #11 from this note. This does not change the JWT dogfood or drafts #1 and #2.
- **Next action:**
  1. **JWT-auth-style dogfood** in a real repo — acceptance bar for the invent path (PR #9 checklist and `docs/MISSION_CONTROL_LAB_CONTRACT.md` §12). Still not executed.
     ...
     - `docs/MISSION_CONTROL_LAB_CONTRACT.md` §12 also requires a multi-step graph, concurrent independent ready branches, and the attempt stack (including a retry or fixer) on the same step.
