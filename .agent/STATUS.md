...
- **This work:** Gap-fill plus thin-vertical scaffolding on the #2 line, branched from PR #2 head with current main merged in. Branch `cursor/graph-breakers-gap-fill-92b3`. Not a third recovery/reconstruction of the graph. `mad_scientist_graph.py` and the `mad_scientist_*` tables stay the PR #2 scheduler and DDL, with columns and behavior added on top.
...
- **Next action:** Jeffrey reviews the gap-fill draft PR against main. Do not merge or close #1 or #2. After review, run the manual JWT-auth dogfood in a real repo. Lab creations still execute only as Mission Control attempts.
